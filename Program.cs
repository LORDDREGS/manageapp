...
app.MapGet("/api/users/{id}", async (string id) =>
{
    ...
    var user = await usersCollection.Find(u => u.Id == objectId).FirstOrDefaultAsync();
...
{
    user.Id = ObjectId.GenerateNewId(); // Генерация нового ID
...
app.MapDelete("/api/users/{id}", async (string id) =>
{
    ...
    var result = await usersCollection.FindOneAndDeleteAsync(u => u.Id == objectId);
    return result is null ? Results.NotFound(new { message = "User not found." }) : Results.Ok(result);
});
